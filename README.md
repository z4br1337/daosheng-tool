...
- **学号注册 + 班级选择**：用户注册时先选择已存在的班级；如果没有班级，管理员需要先新建班级
- **最高管理员账号**：学号 `240153484` 为最高管理员，登录时自动免审核，并默认绑定到测试班级1班
...
浏览器打开 http://localhost:3000，使用学号 **240153484** 登录即可直接进入系统；如需创建最高管理员账号，可先在数据库中预置该学号并设置密码。
...
2. 在服务环境变量中填入上表所有必填项
3. Zeabur 会自动检测 Dockerfile 并构建部署，暴露 8080 端口（镜像内全局安装 `prisma` CLI 后由 root 执行 `db push`，再以非 root 运行应用；避免精简 `node_modules` 缺少 `effect` 等依赖导致 CLI 崩溃）
